...
def build_data_loader(
    ...
    if all(isinstance(item, list) for item in data_source):
        flattened_data = []
        for item in data_source:
            flattened_data.extend(item)
        data_source = flattened_data
...
class DatasetWrapper_train_sf(TorchDataset):
    ...
    def __init__(self, cfg, data_source, transform=None, is_train=False,traindata=None):
        ...
        # self.data = [datum for sublist in self.data_source for datum in sublist]
        # Build transform that doesn't apply any data augmentation
        interp_mode = INTERPOLATION_MODES[cfg.INPUT.INTERPOLATION]
        to_tensor = []
        to_tensor += [T.Resize(cfg.INPUT.SIZE, interpolation=interp_mode)]
    ...
    def __getitem__(self, idx):
        ...
        if self.transform is not None:
            ...
            output["img"] = self.to_tensor(img0)  # Convert PIL Image to Tensor here
